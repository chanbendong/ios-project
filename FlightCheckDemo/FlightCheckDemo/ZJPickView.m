...
#define ZJToobarHeight 40
#import "ZJPickView.h"

@interface ZJPickView ()<UIPickerViewDelegate,UIPickerViewDataSource>
...
-(void)setFrameWith:(BOOL)isHaveNavControler{
    CGFloat toolViewX = 0;
    CGFloat toolViewH = _pickeviewHeight+ZJToobarHeight;
    CGFloat toolViewY ;
...
-(void)setUpPickView{
    ...
    pickView.frame=CGRectMake(0, ZJToobarHeight, kScreenWidth, pickView.frame.size.height);
...
-(void)setUpDatePickerWithdatePickerMode:(UIDatePickerMode)datePickerMode{
    ...
    datePicker.maximumDate = [NSDate dateWithTimeInterval:99999999 sinceDate:datePicker.minimumDate];
    datePicker.backgroundColor=[UIColor whiteColor];
    if (_defaulDate) {
        [datePicker setDate:_defaulDate];
    }
    _datePicker=datePicker;
    datePicker.frame=CGRectMake(0, ZJToobarHeight, kScreenWidth, datePicker.frame.size.height);
...
-(void)setToolbarWithPickViewFrame{
    _toolbar.frame=CGRectMake(0, 0,[UIScreen mainScreen].bounds.size.width, ZJToobarHeight);
}
...
-(void)doneClick
{
    if (_pickerView) {
        ...
        NSDateComponents *comps = [calendar components:(NSCalendarUnitHour | NSCalendarUnitMinute | NSCalendarUnitSecond) fromDate:[[NSDate alloc]init]];
        [comps setHour:0];
        [comps setMinute:0];
        [comps setSecond:0];
        NSDate *nowDate = [calendar dateByAddingComponents:comps toDate:_datePicker.date options:0];
        _resultString=[NSString stringWithFormat:@"%@",nowDate];
